...
export class PdfService {
  ...
  public async generateDiagnosticPdf(
    ...
    // Obter os significados específicos para a combinação de IA/Cultura
    const companyInsights = this.getCompanyMeaning(iaLevel, culturaLevel);
    
    // Nome da empresa do formulário, se disponível
    const companyName = options.company || formData?.empresa || 'Sua empresa';
    
    // Adicionar cada item da lista com bullet point
    companyInsights.forEach(point => {
      doc.fontSize(12)
         .font('Helvetica')
         .fillColor('#333333')
